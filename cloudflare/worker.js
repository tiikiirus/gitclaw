...
class MistralAdapter {
  ...
  prepareRequest(modelId, originalBody, env, key) {
    ...
    const configuredModel =
      (env.MISTRAL_MODEL || "").trim() || DEFAULT_MISTRAL_MODEL;
    const requestedModel =
      !modelId || modelId === "auto" ? configuredModel : modelId;
    if (!isMistralModel(requestedModel)) return null;
    const baseUrl = (env.MISTRAL_BASE_URL || DEFAULT_MISTRAL_BASE_URL)
      .trim()
      .replace(/\/+$/, "");
...
class OpenCodeAdapter {
  ...
  prepareRequest(modelId, originalBody, env, key) {
    ...
    if (keys.length === 0) return null;
    const configuredModel =
      (env.OPENCODE_MODEL || "").trim() || DEFAULT_OPENCODE_MODEL;
    const requestedModel =
      !modelId || modelId === "auto" ? configuredModel : modelId;
  ...
  baseUrl(env) {
    const value = (env.OPENCODE_BASE_URL || DEFAULT_OPENCODE_BASE_URL)
      .trim()
      .replace(/\/+$/, "");
...
class TokenRouterAdapter {
  ...
  async fetchModels(env) {
    if (this.getKeys(env).length === 0) return [];
    const model =
      (env.TOKENROUTER_MODEL || "").trim() || DEFAULT_TOKENROUTER_MODEL;
    return [
      {
        id: model,
        providerId: this.id,
  ...
  prepareRequest(modelId, originalBody, env, key) {
    ...
    const baseUrl = (env.TOKENROUTER_BASE_URL || DEFAULT_TOKENROUTER_BASE_URL)
      .trim()
      .replace(/\/+$/, "");
    const chatBase = baseUrl.toLowerCase().endsWith("/v1")
      ? baseUrl
